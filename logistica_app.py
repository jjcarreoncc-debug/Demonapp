...
def estructura_archivos_logistica():
    ...
    st.info(
        "📌 Los archivos Excel deben venir en forma horizontal."
    )

    with st.expander(
        "📄 Estructura requerida archivos Logística"
    ):
...
def logistica_app():
    ...
    # =========================
    if archivo_transito:
        st.session_state.df_transito = pd.read_excel(
            archivo_transito
        )

    if archivo_bodegas:
        st.session_state.df_bodegas_log = pd.read_excel(
            archivo_bodegas
        )

    if archivo_transportistas:
        st.session_state.df_transportistas = pd.read_excel(
            archivo_transportistas
        )

    if archivo_rutas:
        st.session_state.df_rutas = pd.read_excel(
            archivo_rutas
        )

    if archivo_recepcion:
        st.session_state.df_recepcion = pd.read_excel(
            archivo_recepcion
        )

    if archivo_despachos:
        st.session_state.df_despachos = pd.read_excel(
            archivo_despachos
        )

    transito = st.session_state.get(
        "df_transito"
    )
    ...
    transportistas = st.session_state.get(
        "df_transportistas"
    )

    rutas = st.session_state.get(
        "df_rutas"
    )

    recepcion = st.session_state.get(
        "df_recepcion"
    )

    despachos = st.session_state.get(
        "df_despachos"
    )
    ...
    if (
        ...
        st.warning(
            "⚠️ Carga todos los archivos Excel de Logística"
        )
        return
