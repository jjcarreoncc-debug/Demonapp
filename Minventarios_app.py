...
# =========================
# PANTALLA: ALTA DE MATERIAL
# =========================
def pantalla_alta_material():

    st.title("➕ Alta de material")
    st.caption("Maestros / Productos / Maestro de materiales / Alta de material")
    ...
    with st.form("form_alta_material"):
        ...
        with c1:
            ...
            categoria = st.selectbox(
                "Categoría",
                ["Materia prima", "Producto terminado", "Insumo", "Refacción", "Servicio"]
            )
        ...
        with c2:
            ...
            tipo_material = st.selectbox(
                "Tipo material",
                ["Almacenable", "No almacenable", "Servicio", "Kit"]
            )
            estatus = st.selectbox(
                "Estatus",
                ["Activo", "Bloqueado", "Descontinuado"]
            )
            unidad_base = st.selectbox(
                "Unidad base",
                ["PZA", "KG", "LT", "CJ", "MT", "SERV"]
            )
        ...
        with c1:
            peso = st.number_input("Peso", min_value=0.0, step=0.01)
            volumen = st.number_input("Volumen", min_value=0.0, step=0.01)
            largo = st.number_input("Largo", min_value=0.0, step=0.01)

        with c2:
            ancho = st.number_input("Ancho", min_value=0.0, step=0.01)
            alto = st.number_input("Alto", min_value=0.0, step=0.01)
            tipo_almacenamiento = st.selectbox(
                "Tipo almacenamiento",
                ["General", "Refrigerado", "Congelado", "Peligroso", "Alto valor"]
            )

        with c3:
            almacen_default = st.text_input("Almacén default")
            ubicacion_default = st.text_input("Ubicación default")
            rotacion_abc = st.selectbox("Clasificación ABC", ["A", "B", "C"])
        ...
        with c1:
            costo_estandar = st.number_input("Costo estándar", min_value=0.0, step=0.01)
        ...
        with c2:
            ...
            moneda = st.selectbox("Moneda", ["MXN", "USD", "EUR", "COP"])

        with c3:
            impuesto = st.selectbox("Impuesto", ["IVA 0%", "IVA 8%", "IVA 16%", "Exento"])
            margen_objetivo = st.number_input("Margen objetivo %", min_value=0.0, step=0.5)
        ...
        with c2:
            punto_reorden = st.number_input("Punto de reorden", min_value=0.0, step=1.0)
            lead_time = st.number_input("Lead time días", min_value=0, step=1)

        with c3:
            permite_negativo = st.checkbox("Permite inventario negativo")
            requiere_inspeccion = st.checkbox("Requiere inspección calidad")
        ...
        with c1:
            ...
            sku_base = st.text_input("SKU base")

        with c2:
            codigo_sap = st.text_input("Código SAP / externo")
            proveedor_principal = st.text_input("Proveedor principal")

        st.markdown("---")

        guardar = st.form_submit_button("💾 Guardar material")

        if guardar:
            st.success("✅ Material listo para guardar en base de datos.")
            st.write({
...
if opcion_inv == "Alta de material":
    pantalla_alta_material()

elif opcion_inv == "Baja de material":
    pantalla_placeholder("❌ Baja de material")

elif opcion_inv == "Consulta de material":
    pantalla_placeholder("🔍 Consulta de material")

elif opcion_inv == "Alta de SKU":
    pantalla_placeholder("➕ Alta de SKU")

elif opcion_inv == "Baja de SKU":
    pantalla_placeholder("❌ Baja de SKU")

elif opcion_inv == "Consulta de SKU":
    pantalla_placeholder("🔍 Consulta de SKU")
