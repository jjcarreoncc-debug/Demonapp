...
def procesar_datos(productos, movimientos, inventario):
    ...
    stock = (
        movimientos
        .groupby("NUMERO_PRODUCTO")[["ENTRADA", "SALIDA"]]
        .sum()
        .reset_index()
    )
...
def calcular_metricas(df):
    ...
    metricas["criticos"] = (
        df[df["STOCK"] < df["STOCK_MIN"]]
        .shape[0]
    )

    metricas["sobrestock"] = (
        df[df["STOCK"] > df["STOCK_MAX"]]
        .shape[0]
    )
...
def inventarios_app():
    ...
    archivo_prod = st.file_uploader(
        "📦 Productos",
        type=["xlsx"],
        key="prod"
    )

    archivo_mov = st.file_uploader(
        "📊 Movimientos",
        type=["xlsx"],
        key="mov"
    )
